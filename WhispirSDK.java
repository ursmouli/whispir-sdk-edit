...
public class WhispirSDK implements MessageHelper, WorkspaceHelper,
	...
	public WhispirResponse post(String resourceType, String resourceId, String workspaceId,
		...
		System.out.println("post: resourceType=" + resourceType + ", resourceId=" + resourceId + ", workspaceId=" + workspaceId + ", jsonContent=" + jsonContent);
	...
	public WhispirResponse get(String resourceType, String workspaceId, String resourceId, MessageFilter filter)
			throws WhispirSDKException {
		System.out.println("get: resourceType=" + resourceType + ", resourceId=" + resourceId + ", workspaceId=" + workspaceId);
		HttpGet httpGet = (HttpGet) createGet(resourceType, workspaceId, resourceId, filter);
	...
	private HttpRequestBase createGet(String resourceType, String workspaceId, String resourceId)
		...
		// Create a method instance.
		String url = buildUrl(workspaceId, resourceType, resourceId);
		System.out.println("createGet url = " + url);
	...
	private HttpRequestBase createGet(String resourceType, String workspaceId, String resourceId, MessageFilter filter)
			throws WhispirSDKException {
		// Create a method instance.
		String url = buildUrl(workspaceId, resourceType, resourceId, filter);
	...
	private void setHeaders(HttpRequestBase request, String resourceType)
		...
		//case WhispirSDKConstants.MESSAGES_RESOURCE:
			if (resourceType.equals(WhispirSDKConstants.MESSAGES_RESOURCE)){
				header = WhispirSDKConstants.WHISPIR_MESSAGE_HEADER_V1;
			}
			//break;

		//case WhispirSDKConstants.WORKSPACES_RESOURCE:
			else if (resourceType.equals(WhispirSDKConstants.WORKSPACES_RESOURCE)){
				header = WhispirSDKConstants.WHISPIR_WORKSPACE_HEADER_V1;
			//break;
			}

		//case WhispirSDKConstants.SCENARIOS_RESOURCE:
			else if (resourceType.equals(WhispirSDKConstants.SCENARIOS_RESOURCE)){
	...
	private String buildUrl(String workspaceId, String resourceType,
			String resourceId, MessageFilter filter) {
		...
		if (filter != null && filter.isValid()) {
			if (filter.getLimit() > 0) {
				url.append(WhispirSDKConstants.LIMIT_EXT + filter.getLimit());
			}
			if (filter.getOffset() >= 0) {
				url.append(WhispirSDKConstants.OFFSET_EXT + filter.getOffset());
			}
			if (filter.getFromDate() != null && !"".equals(filter.getFromDate())) {
				url.append(WhispirSDKConstants.FROM_DATE_EXT + filter.getFromDate());
			}
			if (filter.getFromTime() != null && !"".equals(filter.getFromTime())) {
				url.append(WhispirSDKConstants.FROM_TIME_EXT + filter.getFromTime());
			}
			if (filter.getToDate() != null && !"".equals(filter.getToDate())) {
				url.append(WhispirSDKConstants.TO_DATE_EXT + filter.getToDate());
			}
			if (filter.getToTime() != null && !"".equals(filter.getToTime())) {
				url.append(WhispirSDKConstants.TO_TIME_EXT + filter.getToTime());
			}
		}
